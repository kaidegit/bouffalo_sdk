...
DUT_USAGE_MARKER = "Usage: pds <pds_mode> <test_io> <trig_type> <pull>"
DUT_READY_MARKER = "GPIO Ready"
DUT_WAKEUP_RE = re.compile(r"gpio_(\d+) wakeup pds")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="One-click BL616CL PDS GPIO wakeup validation: DUT build + flash + DFT/DUT serial test"
    )
    parser.add_argument(
        "--config",
        default="pds_wakeup_validation_config.example.json",
        help="Config JSON path. Default: pds_wakeup_validation_config.example.json",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
...
def arm_dut_pds(
    ...
    command = f"pds {pds_mode} {pin} {trigger.trig_type} {trigger.pull}"
    required = [
        f"pds_mode: PDS_{pds_mode}",
        f"test_io: {pin}",
        f"test_trig_type:{trigger.description}",
        f"pull: {trigger.pull_text}",
    ]
    failure_patterns = ["test_io must be less than", "pull must be less than", "[ERR]", DUT_USAGE_MARKER]
    last_error: Exception | None = None
    for attempt in range(1, retries + 2):
        try:
...
def run_single_case(
    ...
    start_message = (
        f"case={case_index}/{total_cases} pds={pds_mode} dut_gpio={pair.dut_gpio} "
    ...
    try:
        ...
        dut_result, attempt = arm_dut_pds(
            dut,
            pds_mode,
            pair.dut_gpio,
            trigger,
...
def run_validation(
    ...
    total_cases = len(pds_modes) * len(pins) * len(triggers)
    case_index = 0
...
def build_result_workbook(records: list[TestRecord], output_path: Path, trigger_order: list[str]) -> None:
    ...
    for record in records:
        grouped.setdefault(record.pds_mode, []).append(record)

    for pds_mode in sorted(grouped):
        sheet = workbook.create_sheet(f"pds{pds_mode}")
        ...
        row = 2
        rows = sorted(grouped[pds_mode], key=lambda item: (item.dut_gpio, order_map[item.trigger_name]))
...
def main() -> int:
    ...
    tests = raw.get("tests", {})
    pds_modes = [int(item) for item in tests.get("pds_modes", [1, 3, 7, 15])]
    ...
    log_path = output_dir / "pds_wakeup_validation.log"
    report_path = output_dir / "pds_wakeup_validation_report.xlsx"
    result_json_path = output_dir / "pds_wakeup_validation_results.json"
